---
title: "EHR SOFA --- Table 2a and 3b, and more"
subtitle: "Sensitivity analysis, excluded patients with ESRD at admission"
...
```{r}
FirstMV.select <- FirstMV.select.NotESRD
FirstMVdata.train<- FirstMVdata.train.NotESRD
FirstMVdata.test<- FirstMVdata.test.NotESRD
...
print(my_basetable, target = paste0("Characteristics table of patients started intubation after admission_removedESRD.docx"))
...
pdf("CalibrationPlot_OnTrainTest_RemovedESRD.pdf", height = 8, width = 8)
...
pdf("ROC for mortality among ventilated train_RemovedESRD.pdf", width = 8, height = 8)

ggroc(list("SOFA (AUC=65.6%)" = roc.train.sofa,
           "SOFA Categories (AUC=54.1%)" = roc.train.sofaCat,
           "Age (AUC=71.2%)" = roc.train.age,
           "SOFA + Age (AUC=74.6%)" = roc.train.sofaAge
           ), 
...
ggroc(list("SOFA (AUC=65.8%)" = roc.test.sofa,
           "SOFA Categories (AUC=53.63%)" = roc.test.sofaCat,
           "Age (AUC=71.3%)" = roc.test.age,
           "SOFA + Age (AUC=74.3%)" = roc.test.sofaAge), size=1.5, aes = c("linetype","color"), alpha=1) +
  theme_bw(base_size = 14) +
  geom_abline(slope = 1, intercept =100)+
  scale_color_manual(values =c("#440154FF", "#6DCD59FF","orange", "royalblue"))+
  scale_linetype_manual(values=c(1,1,1,1))+
